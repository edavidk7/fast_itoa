...
#define MAX_N_DIGITS 3
#define DENOM 1000

// Implemented from and optimized
// https://kenny-peng.com/2021/05/28/printing_integers_fast.html

inline size_t count_digits32(uint32_t value);

const char table_3_digits[3000] __attribute__((aligned(64))) = "000001002003004005006007008009"
...
const char table_2_digits[200] __attribute__((aligned(64))) = "00010203040506070809"
...
inline size_t int2a(int32_t value, char *dest) {
  size_t idx = 0;
  if (value < 0) {
    value = -value;
    dest[idx++] = '-';
  }
  return idx + uint2a(value, dest + idx);
}

inline size_t uint2a(uint32_t value, char *dest) {
  size_t idx = 0;
  if (value < 10) {
    dest[idx++] = value + '0';
  } else {
    int n_digits = count_digits32(value);
    idx += n_digits;
    char *temp_ptr = dest + idx;
    while (n_digits >= 3) {
      temp_ptr -= 3;
      n_digits -= 3;
      int q = value / 1000;
      memcpy(temp_ptr, table_3_digits + 3 * (value - 1000 * q), 3);
      value = q;
    }
    if (value) {
      temp_ptr -= n_digits;
      if (n_digits > 1) {
        memcpy(temp_ptr, table_2_digits + 2 * value, 2);
      } else {
        *(temp_ptr) = value + '0';
      }
    }
